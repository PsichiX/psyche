...
use psyche::core::brain::{activity, BrainActivityStats};
...
use psyche::serde::json::{brain_builder_from_json, brain_from_json};
use psyche::serde::yaml::{brain_builder_from_yaml, brain_from_yaml};
...
fn main() -> Result<()> {
    ...
    let timeline = if let Some(timeline) = matches.value_of("timeline") {
    ...
    while let Some(actions) = timeline.perform(last_time, next_time) {
        println!("Rendering {} -> {}", last_time, next_time);
        if verbose {
        ...
        for action in actions {
            match action.action_type {
                ...
                ActionType::IgniteRandomSynapsesByPercentage(percentage, (min, max)) => {
                    ...
                    drop(brain.ignite_random_synapses(count, (min, max)));
                }
